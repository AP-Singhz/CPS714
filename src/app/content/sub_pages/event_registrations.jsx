...
const EventRegistrationManagmentTable = ({updateAddRegistration, updateSelectedRegistration}) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try{
                const qreg = query(collection(db, 'registrations'), orderBy('registration_id', 'desc')); 
                ...
                <tbody>
                    {registrations.map(registration => (
                        <tr className = "hover:border-slate-300 hover:text-black hover:border-2 hover:shadow hover:bg-violet-100 hover:font-semibold" key={registration.id}>
                            ...
                            <td className="py-2 px-4 border text-center overflow-hidden w-1/12">{new Date(registration.event_date.seconds * 1000).toLocaleString()}</td>
    ...
    export default EventRegistrationManagmentTable;